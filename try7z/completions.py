...
_try7z_completion() {
    ...
    case "${cmd}" in
        ...
        autocompletion)
            case "${prev}" in
                --shell)
                    COMPREPLY=( $(compgen -W "bash pwsh" -- "${cur}") )
                    ;;
                *)
...
def generate_pwsh_completion() -> str:
    """Generate PowerShell completion script for try7z.
    ...
    Example:
        ...
        True
    """
    return r"""# try7z PowerShell completion script
# Generated automatically - do not edit manually
...
def _get_pwsh_profile_path() -> Path:
    ...
    Returns:
        Path to PowerShell profile file.
    """
    if sys.platform == "win32":
        # Try to get profile path via PowerShell
        try:
        ...
        config_dir = Path.home() / ".config" / "powershell"
        return config_dir / "Microsoft.PowerShell_profile.ps1"
...
def install_pwsh_completion() -> None:
    ...
    Appends the completion script to the PowerShell profile file.
    ...
    if profile.exists():
        ...
        profile.write_text(completion_script + "\n", encoding="utf-8")
...
def install_completion(shell: str) -> None:
    ...
    Args:
        shell: Shell type, either "bash" or "pwsh".
    ...
    if shell == "bash":
        ...
        raise ValueError(
            f"Unsupported shell: {shell}. Use 'bash' or 'pwsh'."
        )
